 ...
 * @opcode Opcode.CALL
 */
export function call(state: State, hostFunctions: Map<string, any>, arg: any): void
{
    const operand: {
        name: string,
    ...
    if (operand.name && hostFunctions.has(operand.name)) {
        const args: any[] = [];
        for (let i = 0; i < operand.args; i++) {
            args.unshift(state.pop());
        }

        const result = hostFunctions.get(operand.name)!(...args);
